...
def generate_cif_file(df, motif_name, base_dir):
    ...
    with open(file_path, 'w') as file:
        ...
        file.write('_audit_creation_method \'Generated by DataFrame to CIF conversion\'\n')
        file.write('\n')
        file.write('loop_\n')
        ...
        for index, row in df.iterrows():
            file.write(
                "{:<6}{:<6}{:<6}{:<6}{:<6}{:<6}{:<12}{:<12}{:<12}\n".format(
                    row['group_PDB'], row['id'], row['auth_atom_id'],
                    row['auth_asym_id'], row['auth_seq_id'], row['pdbx_PDB_ins_code'],
