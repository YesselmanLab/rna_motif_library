...
def get_dssr_motifs(pdb_ids: List[str], processes: int = 1):
...
def compare_dssr_motifs(pdb_ids: List[str], processes: int):
    os.makedirs(
...
def run_compare_dssr_motifs(csv_path, processes):
...
def get_atlas_motifs_summary(csv_paths):
    ...
    df = pd.concat(dfs)
    df.to_json(
        os.path.join(DATA_PATH, "summaries", "other_motifs", "atlas_motifs.json"),
        orient="records",
    )
...
def run_compare_atlas_motifs(csv_path, processes):
